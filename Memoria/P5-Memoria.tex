...
%% Inicio del documento
	...
	considerado ahorrar su explicación en este documento para permitir una mayor limpieza del mismo.}
		...
		\subsection{Algoritmo Memético Parcial (\textbf{PMA})}
			...
			la probabilidad de mejora. Despúes, realiza la búsqueda local de baja intensidad. Esto queda
			claramente reflejado en el siguiente algoritmo:
		...
		\subsection{Algotitmo Memético Parcial Elitista (\textbf{EPMA})}
			...
			\label{EPMA-Improve}
			\end{algorithm}\footnotetext{Se ha separado el condicional en dos partes para facilitar la lectura
			de la condición que lo lanza.}
			
			En este caso, también tomamos una probabilidad de mejora de 0.1 para nuestro algoritmo elitista.
